...
import { KIND_LABEL, prepare, search, type SearchDoc, type SearchResult } from "@/lib/search/search";

const EXAMPLES = ["recibo", "renda", "IRS", "abono", "burla", "reforma", "desemprego", "IMI"];

/** Reads ?q= from the address and searches the index in the browser. */
export function SearchFromUrl({ docs }: { docs: SearchDoc[] }) {
  const q = (useSearchParams().get("q") ?? "").slice(0, 120).trim();
  const index = useMemo(() => prepare(docs), [docs]);
  const { results, partial } = useMemo(() => (q ? search(index, q) : { results: [], partial: false }), [index, q]);
  return <SearchView q={q} results={results} partial={partial} />;
}

export function SearchView({ q, results, partial }: { q: string; results: SearchResult[]; partial: boolean }) {
  return (
    <>
      ...
      <PageHeader title="Pesquisar" lead="Escreva uma palavra ou uma pergunta. Pode escrever sem acentos.">
        <SearchForm key={q} id="pesquisa-q" size="lg" defaultValue={q} autoFocus={!q} className="mt-6 max-w-2xl" />
      </PageHeader>
